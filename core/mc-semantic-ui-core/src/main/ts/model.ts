...
// ── Triggers ────────────────────────────────────────────────────────────────
// Mirrors UiTrigger.java. The renderer doesn't dispatch triggers itself; it
// just serialises them into data-trigger attributes for the central click
// dispatcher to pick up.

// Mirrors UiTrigger.Behavior. Extension apps can register additional
// behaviours on the SuiEventBus, so this string union is widened on the
// trigger object itself — see {@link UiTrigger#behavior}.
export type TriggerBehavior =
    | "APPLY_RESPONSE" | "STREAM" | "DOWNLOAD" | "OPEN_IN_TAB" | "INVOKE" | "PATCH" | "UPLOAD";
...
// ── Leaf shapes (not top-level nodes) ───────────────────────────────────────
