...
| `TIME` | `<input type="time">` | `min`, `max` — `HH:mm`; `step` in seconds, or `.minutes(15)`: the field carries a datalist of exactly those times, which is what makes the browser's picker offer only them (its own minute list ignores `step`), and a value typed in between is rounded to the nearest one. `UiField.time(id, label, "08:30")`, or a `LocalTime` |
...
sets the step in a readable way. The field then carries a `<datalist>` of
exactly those times — the browser's own picker lists every minute whatever
`step` says, and only the datalist makes it offer the stepped ones — and a
value typed in between — `09:07` at quarter hours — is rounded to
